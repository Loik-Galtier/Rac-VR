...
namespace AmplifyShaderEditor
{
	...
	public class UsePassHelper : ScriptableObject
	{
        ...
        private void DisplayShaderContext(UndoParentNode node, Rect r)
        {
            ...
            ShaderInfo[] shaderInfos = ShaderUtil.GetAllShaderInfo(); //correction there

            GenericMenu shaderMenu = new GenericMenu();

            foreach (var shaderInfo in shaderInfos)
            {
                if (!string.IsNullOrEmpty(shaderInfo.name))
                {
                    shaderMenu.AddItem(new GUIContent(shaderInfo.name), false, () =>
                    {
                        OnSelectedShaderPopup(shaderInfo.name, Shader.Find(shaderInfo.name));
                    });
                }
            }
